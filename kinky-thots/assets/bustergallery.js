...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    console.log('Buster Gallery initialized');
    loadGallery();
    setupLightbox();
  });

  async function loadGallery() {
    const grid = document.getElementById('gallery-grid-buster');
    if (!grid) return;

    grid.innerHTML = '<div class="loading">Loading gallery...</div>';

    try {
      const response = await fetch(CONFIG.apiBase + CONFIG.endpoints.gallery + '?page=buster');
      ...
      if (isVideo) {
        return `
          <div class="gallery-item video-item" data-idx="${idx}">
            <video
              src="${fileUrl}"
              onclick="window.openBusterLightbox(${idx})"
  ...
  function setupLightbox() {
    const overlay = document.getElementById('lightbox-overlay-buster');
    const overlayImg = document.getElementById('lightbox-img-buster');
    const overlayVideo = document.getElementById('lightbox-video-buster');
    const closeBtn = document.getElementById('lightbox-close-buster');
    const prevBtn = document.getElementById('lightbox-prev-buster');
    const nextBtn = document.getElementById('lightbox-next-buster');

    if (!overlay) return;

    window.openBusterLightbox = function(idx) {
    ...
    function showPrev() {
      if (currentLightboxIndex > 0) {
        window.openBusterLightbox(currentLightboxIndex - 1);
      }
    }
