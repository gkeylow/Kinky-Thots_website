# Kinky-Thots — Full History & Details

> Detailed changelog and reference. Only read this file when you need deep context (version history, architecture details, past changes). Essential info is in [CLAUDE.md](./CLAUDE.md).
...
|---------|------|------------|
| 1.9.5 | Feb 8, 2026 | Redirect live.html → Owncast, compact nav, mobile volume fix |
...
│   │   ├── (live.js removed - backed up in /backup/live-page/)
...
│       ├── (live.css removed - backed up in /backup/live-page/)
│       └── chat.css  # Chat component styles (unused, Owncast handles chat)
...
- `live.html` - Redirect to https://owncast.kinky-thots.xxx (old page backed up in /backup/live-page/)
...
6. **live.html** redirects to Owncast (previous embed approach removed)
...
### Redirect live.html to Owncast + Compact Nav (v1.9.5)
**Status: Complete and deployed**

Abandoned embedded Owncast approach (iframe layout issues on mobile) in favor of direct redirect to Owncast.

**What changed:**
- `live.html` replaced with instant `<meta http-equiv="refresh">` redirect to `https://owncast.kinky-thots.xxx`
- All "Live Cam" nav links across 12+ pages updated to link directly to Owncast (`target="_blank"`)
- `reset-password.html` redirects changed from `/live.html` to `/index.html`
- Removed `src/js/live.js`, `src/css/live.css`, `src/css/live.css.save` (source files)
- Removed `live` entry from `vite.config.js`
- All original files backed up to `/backup/live-page/`

**Nav/Header made compact (matching Owncast header):**
- `header.css`: nav padding `1rem 0` → `0.3rem 0`, logo `50px` → `32px`, font `1.5rem` → `1.2rem`, link padding `0.75rem 1.5rem` → `0.5rem 1.1rem`, link font `1rem` → `0.9rem`
- `content.css`: adjusted all `padding-top` offsets for smaller nav (~48px vs ~80px)

**Owncast custom CSS updates:**
- Added mobile volume indicator fix: hides `.vjs-volume-panel` on touch/mobile devices via `@media (max-width: 1024px), (hover: none), (pointer: coarse)`

**Backup directory:** `/backup/live-page/`
- `live.html`, `live.css`, `live.css.save`, `live.js` (source files)
- `live.dist.css`, `live.dist.js` (built files)
- `vite-entry.txt` (removed vite config line)
...
### live.html (Originally Converted to Chat-Only, Now Redirect)
- v1.9.4: Converted to Owncast embed with chat iframes
- v1.9.5: Replaced with redirect to Owncast (embed layout had mobile scroll issues)
- Original files backed up to `/backup/live-page/`
...
- Webui: https://mail.kinky-thots.com (admin / iMaTiOn@081903)
...
| `live.css` | (Removed - backed up) | Was live.html |
...
| live.html | Redirect to Owncast (no CSS) |
