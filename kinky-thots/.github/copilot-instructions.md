...
Video manifest at `data/video-manifest.json` replaces file scanning in `porn.php`.
...
4. **No Local Video Scanning**: `porn.php` prefers manifest over directory scanning (lines 11-18) to avoid filesystem I/O for large media libraries
...
| `porn.php` | Video gallery renderer using manifest |
...
2. Update `getMimeType()` in `porn.php` if needed
